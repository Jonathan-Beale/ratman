...
def yolov12_inference(image, video, model_id, image_size, conf_threshold):
    model = YOLO(model_id)
    if image:
        results = model.predict(source=image, imgsz=image_size, conf=conf_threshold, device="mps")
        ...
        while cap.isOpened():
            ...
            results = model.predict(source=frame, imgsz=image_size, conf=conf_threshold, device="mps")
...
def app():
    with gr.Blocks():
        with gr.Row():
            with gr.Column():
                input_type = gr.Radio(choices=["Image", "Video"], label="Input Type")
